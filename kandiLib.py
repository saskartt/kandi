...
def averageProfilesVariances(statdomain, t_inds, h_inds, ds):
  # Variances
  var_u = np.mean(ds.variables['u*2'][t_inds,:],axis=0)
  var_u = np.interp(h_inds, ds.variables['zu*2'][:], var_u)

  var_v = np.mean(ds.variables['v*2'][t_inds,:],axis=0)
  var_v = np.interp(h_inds, ds.variables['zv*2'][:], var_v)

  var_w = np.mean(ds.variables['w*2'][t_inds,:],axis=0)
  var_w = np.interp(h_inds, ds.variables['zw*2'][:], var_w)
...
def averageProfilesTKE(statdomain, t_inds, h_inds, ds):
  ...
  tke_e = np.mean(ds.variables['e*'][t_inds,:],axis=0)
  tke_e = np.interp(h_inds, ds.variables['ze*'][:], tke_e)
  tke_e_sg = np.mean(ds.variables['e'][t_inds,:],axis=0)
  tke_e_sg = np.interp(h_inds, ds.variables['ze'][:], tke_e_sg)
...
def compareProfilesWS(statdomain, t_inds, ct_inds, h_inds, ds, cds):
  u = np.mean(ds.variables['u'][t_inds,:],axis=0)
  u = np.interp(h_inds, ds.variables['zu'][:], u)
  cu = np.mean(cds.variables['u'][ct_inds,:],axis=0)
  cu = np.interp(h_inds, ds.variables['zu'][:], cu)
  cmp_u = createTableRow(np.divide(u,cu)*100.,'u (%)')

  v = np.mean(ds.variables['v'][t_inds,:],axis=0)
  v = np.interp(h_inds, ds.variables['zv'][:], v)
  cv = np.mean(cds.variables['v'][ct_inds,:],axis=0)
  cv = np.interp(h_inds, ds.variables['zv'][:], cv)
  cmp_v = createTableRow(np.divide(v,cv)*100.,'v (%)')

  w = np.mean(ds.variables['w'][t_inds,:],axis=0)
  w = np.interp(h_inds, ds.variables['zw'][:], w)
  cw = np.mean(cds.variables['w'][ct_inds,:],axis=0)
  cw = np.interp(h_inds, ds.variables['zv'][:], cw)
...
def compareProfilesVariances(statdomain, t_inds, ct_inds, h_inds, ds, cds):
  var_u = np.mean(ds.variables['u*2'][t_inds,:],axis=0)
  var_u = np.interp(h_inds, ds.variables['zu*2'][:], var_u)
  var_cu = np.mean(cds.variables['u*2'][ct_inds,:],axis=0)
  var_cu = np.interp(h_inds, ds.variables['zu*2'][:], var_cu)
  cmp_var_u = createTableRow(np.divide(var_u,var_cu)*100.,'u*2 (%)')

  var_v = np.mean(ds.variables['v*2'][t_inds,:],axis=0)
  var_v = np.interp(h_inds, ds.variables['zv*2'][:], var_v)
  var_cv = np.mean(cds.variables['v*2'][ct_inds,:],axis=0)
  var_cv = np.interp(h_inds, ds.variables['zv*2'][:], var_cv)
  cmp_var_v = createTableRow(np.divide(var_v,var_cv)*100.,'v*2 (%)')

  var_w = np.mean(ds.variables['w*2'][t_inds,:],axis=0)
  var_w = np.interp(h_inds, ds.variables['zw*2'][:], var_w)
  var_cw = np.mean(cds.variables['w*2'][ct_inds,:],axis=0)
  var_cw = np.interp(h_inds, ds.variables['zw*2'][:], var_cw)
...
def compareProfilesMomentumFluxes(statdomain, t_inds, ct_inds, h_inds, ds, cds):
  flx_u = np.mean(ds.variables['wu'][t_inds,:],axis=0)
  flx_u = np.interp(h_inds, ds.variables['zwu'][:], flx_u)
  flx_cu = np.mean(cds.variables['wu'][ct_inds,:],axis=0)
  flx_cu = np.interp(h_inds, ds.variables['zwu'][:], flx_cu)
  cmp_flx_u = createTableRow(np.divide(flx_u,flx_cu)*100.,'wu (%)')

  flx_v = np.mean(ds.variables['wv'][t_inds,:],axis=0)
  flx_v = np.interp(h_inds, ds.variables['zwv'][:], flx_v)
  flx_cv = np.mean(cds.variables['wv'][ct_inds,:],axis=0)
  flx_cv = np.interp(h_inds, ds.variables['zwv'][:], flx_cv)
...
def compareProfilesTKE(statdomain, t_inds, ct_inds, h_inds, ds, cds):
  ...
  tke_ce = np.mean(cds.variables['e*'][ct_inds,:],axis=0)
  tke_ce = np.interp(h_inds, cds.variables['ze*'][:], tke_ce)
  tke_ce_sg = np.mean(cds.variables['e'][ct_inds,:],axis=0)
  tke_ce_sg = np.interp(h_inds, cds.variables['ze'][:], tke_ce_sg)
...
def interpolateScalarField(var, x_dims, y_dims, x_dims_i, y_dims_i):
  ...
  zf = interp2d(x_dims,y_dims,var, kind='linear')
